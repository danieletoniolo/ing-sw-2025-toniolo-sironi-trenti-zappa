package it.polimi.ingsw.view.tui.states.validationScreens;

import it.polimi.ingsw.view.tui.input.Parser;
import it.polimi.ingsw.view.tui.states.TuiScreenView;
import it.polimi.ingsw.view.tui.states.ValidationTuiScreen;

import java.util.function.Supplier;

public class RowAndColValidationTuiScreen extends ValidationTuiScreen {


    public RowAndColValidationTuiScreen() {
        super();

        options.clear();
    }

    @Override
    public void readCommand(Parser parser, Supplier<Boolean> isStillCurrentScreen) throws Exception {
        rowAndCol = parser.getRowAndCol("Type coordinates to destroy a component (row col):", totalLines, isStillCurrentScreen);
    }

    @Override
    public TuiScreenView setNewScreen() {
        tileIDs.add(spaceShipView.removeComponent(rowAndCol.getValue0(), rowAndCol.getValue1()).getID());

        return new ValidationTuiScreen();
    }
}
