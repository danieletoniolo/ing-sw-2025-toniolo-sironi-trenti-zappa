...
class StorageTest {
    ...
    void addGood_addsGoodToStorage() throws IllegalAccessException {
        ...
        PriorityQueue<Good> goods = (PriorityQueue<Good>) goodsField.get(storage);
    ...
    void removeGood_removesGoodFromStorage() throws IllegalAccessException {
        ...
        assertEquals(0, storage.getGoods().size());
    }
    ...
    @RepeatedTest(5)
    void peekGood_whenStorageIsEmpty() {
        Storage storage = new Storage(1, connectors, true, 3);
        assertNull(storage.peekGood(0));
    }

    @RepeatedTest(5)
    void peekGood_whenDepthExceedsSize() {
        Storage storage = new Storage(1, connectors, true, 3);
        Good good = new Good(GoodType.GREEN);
        storage.addGood(good);
        assertThrows(IllegalArgumentException.class, () -> storage.peekGood(2));
    }

    @RepeatedTest(5)
    void peekGood_atGivenDepth() {
        Storage storage = new Storage(1, connectors, true, 3);
        Good good1 = new Good(GoodType.GREEN);
        Good good2 = new Good(GoodType.YELLOW);
        storage.addGood(good1);
        storage.addGood(good2);
        assertEquals(good1, storage.peekGood(1));
        assertEquals(good2, storage.peekGood(0));
        assertEquals(good2, storage.peekGood());
    }

    @RepeatedTest(5)
    void hasGood_whenGoodIsPresent() {
        Storage storage = new Storage(1, connectors, true, 3);
        Good good = new Good(GoodType.GREEN);
        storage.addGood(good);
        assertTrue(storage.hasGood(good));
    }

    @RepeatedTest(5)
    void hasGood_whenGoodIsNotPresent() {
        Storage storage = new Storage(1, connectors, true, 3);
        Good good = new Good(GoodType.GREEN);
        assertFalse(storage.hasGood(good));
    }

    @RepeatedTest(5)
    void pollGood_returnsMostValuableGoodAndRemovesIt() {
        Storage storage = new Storage(1, connectors, true, 3);
        Good good1 = new Good(GoodType.GREEN);
        Good good2 = new Good(GoodType.YELLOW);
        storage.addGood(good1);
        ...
        assertEquals(good2, storage.pollGood());
        assertFalse(storage.getGoods().contains(good2));
        assertEquals(1, storage.getGoods().size());
    }

    @RepeatedTest(5)
    void pollGood_whenStorageIsEmpty() {
        Storage storage = new Storage(1, connectors, true, 3);
        assertNull(storage.pollGood());
    }

    @RepeatedTest(5)
    void pollGood_updatesGoodsValueAndCapacity() throws IllegalAccessException {
        Storage storage = new Storage(1, connectors, true, 3);
        Good good = new Good(GoodType.GREEN);
        storage.addGood(good);
        int initialGoodsValue = (int) goodsValueField.get(storage);
        int initialCapacity = (int) goodsCapacityField.get(storage);

        storage.pollGood();

        assertEquals(initialGoodsValue - good.getValue(), goodsValueField.get(storage));
        assertEquals(initialCapacity + 1, goodsCapacityField.get(storage));
    }
